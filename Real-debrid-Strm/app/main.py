...
class RealDebridManager:
    ...
    async def run(self):
        ...
        try:
            ...
            logger.info(f"   🔄 Cycle Interval: {self.cycle_manager.cycle_interval / 60:.0f} minutes")
            logger.info(f"   📅 File Expiry: {self.cycle_manager.file_expiry_days} days")
            logger.info("")
            ...
            logger.info(f"   ⏱️  Cycles run every {self.cycle_manager.cycle_interval / 60:.0f} minutes")
            logger.info("   🔄 503 errors retry 2 times per cycle")
            logger.info(f"   📅 Files expire after {self.cycle_manager.file_expiry_days} days")
